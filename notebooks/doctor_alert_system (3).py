...
for i, row in enumerate(dataset["train"]):
    try:
        ...
        if not patient_text and not doctor_text:
            print(f"⚠️ Index {i} has no text. Skipping.")
            continue
...
# --- Gmail Configuration ---
GMAIL_USER = "johablack2018@gmail.com"
GMAIL_APP_PASSWORD = "ktcezbanblulflik"  # App Password required
...
def send_alert_smtplib(to_email, subject, message, retries=1):
    ...
    while attempt <= retries:
        try:
            ...
            print(f"📧 Alert sent to {to_email} (Attempt {attempt+1})")
            return True
        except Exception as e:
            print(f"⚠️ Attempt {attempt+1} failed to send email to {to_email}: {e}")
            attempt += 1
...
# --- JSON Alert Loop ---
doctor_email = "kjfariha610@gmail.com"
...
for file in os.listdir(profile_dir):
    if file.endswith(".json"):
        ...
        if risk in ["Medium", "High"]:
            subject = f"⚠️ {risk} Risk Alert from Patient Audio"
            message = f"Detected {risk} risk in patient audio.\n\nTranscript:\n{transcript}"

            success = send_alert_smtplib(doctor_email, subject, message)
            if success:
                print(f"✅ ALERT SENT for {file}: {risk}")
            else:
                print(f"❌ Failed to send alert for {file}: {risk}")

            time.sleep(5)  # small delay to avoid SMTP rate limits
        else:
            print(f"{file} → Low (No alert sent)")
